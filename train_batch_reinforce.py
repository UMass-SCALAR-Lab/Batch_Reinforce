...
def main():
    ...
    # Since Batch Reinforce operates at batch level rather than instance level. We can set it to 1
    group_size = 1
    ...
    for k, prompt_batch in enumerate(prompt_loader):
        ...
        with torch.no_grad():
            for q, a in zip(questions, answers):
                ...
                print(
                    f"rollout q='{q}', a='{a}', returns={returns.sum().item():.2f}, completions: {completions}, replay_buffer_size={len(replay_buffer)}, sequence_ids={sequence_ids.shape}"
                )
                rollout_returns.append(returns.cpu())

                # Dummy Advantage. We dont use this!!!
        ...
        for step_epoch in range(epochs_per_step):
            ...
            for i, exp in enumerate(experience_sampler):
                ...
                batch_returns = exp.returns.flatten()
                ...
                if std.item() < 1e-12:
                    ...
                    # Using Cvxpy
                    info = solve_importance_cvxpy(batch_returns.cpu().numpy(), beta=kl_weight, delta=div_trust_region, log_pi_old_over_ref=kl.cpu().numpy())
                ...
                loss = -((log_probs*action_mask).sum(-1)*imp_ratios).mean()
                ...
                wandb.log({"grad_norm": grad_norm, "var_zero_updates_skipped": var_zero_skip_update_cnt, "cvxpy_fail_updates_skipped": cvxpy_fail_update_cnt})
